...
var _filename, _tempFilename, _locked, _prev, _next, _nextPromise, _nextData, _add, add_fn, _write, write_fn, _filename2, _writer, _adapter, _parse, _stringify;
...
function checkArgs(adapter2, defaultData2) {
  if (adapter2 === void 0)
...
class Low {
  constructor(adapter2, defaultData2) {
    __publicField(this, "adapter");
    __publicField(this, "data");
    checkArgs(adapter2, defaultData2);
...
class Writer {
  constructor(filename) {
    ...
    __privateAdd(this, _tempFilename, void 0);
  ...
  async write(data) {
    return __privateGet(this, _locked) ? __privateMethod(this, _add, add_fn).call(this, data) : __privateMethod(this, _write, write_fn).call(this, data);
  }
}
...
class TextFile {
  constructor(filename) {
    __privateAdd(this, _filename2, void 0);
...
const defaultData = { clients: [], products: [], sales: [], payments: [], expenses: [], suppliers: [], purchases: [], supplier_payments: [], employees: [], advances: [] };
const dbPath = path$1.join(electron.app.getPath("userData"), "db.json");
...
async function deleteItemById(tableName, id) {
  ...
  if ((db.data[tableName] || []).length < initialLength) {
    await db.write();
    return { success: true };
  }
...
async function addClient(client) {
  await db.read();
  const newClient = { id: Date.now(), ...client, initial_debt: client.initial_debt || 0 };
...
async function deleteClient(id) {
  return deleteItemById("clients", id);
}
...
async function updateProduct(id, updatedData) {
  await db.read();
  const item = (db.data.products || []).find((p) => p.id === id);
  if (item) {
...
async function deleteProduct(id) {
  return deleteItemById("products", id);
}
async function getSales() {
  ...
  const salesWithDetails = sales.map((sale) => {
    ...
    return { ...sale, clientName: client ? client.name : "Noma'lum Mijoz", productName: product ? product.name : "Noma'lum Mahsulot" };
  });
  return salesWithDetails.reverse();
}
async function addSale(sale) {
  await db.read();
  const product = (db.data.products || []).find((p) => p.id === sale.productId);
  if (!product)
    throw new Error("Mahsulot topilmadi!");
  const totalPrice = product.price * sale.quantity;
  const newSale = { id: Date.now(), client_id: sale.clientId, product_id: sale.productId, quantity: sale.quantity, total_price: totalPrice, sale_date: (/* @__PURE__ */ new Date()).toISOString() };
  (db.data.sales || []).push(newSale);
  await db.write();
  return newSale;
}
async function deleteSale(id) {
  return deleteItemById("sales", id);
}
async function getPayments() {
  ...
  const paymentsWithDetails = payments.map((payment) => {
    ...
    return { ...payment, clientName: client ? client.name : "Noma'lum Mijoz" };
  });
  return paymentsWithDetails.reverse();
}
async function addPayment(payment) {
  await db.read();
  const newPayment = { id: Date.now(), client_id: payment.clientId, amount: payment.amount, payment_date: (/* @__PURE__ */ new Date()).toISOString() };
  (db.data.payments || []).push(newPayment);
  await db.write();
  return newPayment;
}
async function deletePayment(id) {
  return deleteItemById("payments", id);
}
async function getExpenses() {
  await db.read();
  return (db.data.expenses || []).reverse();
}
async function addExpense(expense) {
  await db.read();
  const newExpense = { id: Date.now(), description: expense.description, amount: expense.amount, expense_date: (/* @__PURE__ */ new Date()).toISOString() };
  (db.data.expenses || []).push(newExpense);
  await db.write();
  return newExpense;
}
async function deleteExpense(id) {
  return deleteItemById("expenses", id);
}
...
async function addPurchase(purchase) {
  await db.read();
  const newPurchase = { id: Date.now(), supplier_id: purchase.supplierId, weight: purchase.weight, price_per_kg: purchase.price_per_kg, total_price: purchase.weight * purchase.price_per_kg, purchase_date: (/* @__PURE__ */ new Date()).toISOString() };
  (db.data.purchases || []).push(newPurchase);
  await db.write();
  return newPurchase;
}
async function addSupplierPayment(payment) {
  await db.read();
  const newPayment = { id: Date.now(), supplier_id: payment.supplierId, amount: payment.amount, payment_date: (/* @__PURE__ */ new Date()).toISOString() };
  (db.data.supplier_payments || []).push(newPayment);
  await db.write();
  return newPayment;
}
async function getSupplierDebts() {
  ...
  const payments = db.data.supplier_payments || [];
  const supplierDebts = suppliers.map((supplier) => {
    const totalPurchases = purchases.filter((p) => p.supplier_id === supplier.id).reduce((sum, current) => sum + current.total_price, 0);
    const totalPayments = payments.filter((p) => p.supplier_id === supplier.id).reduce((sum, current) => sum + current.amount, 0);
    const balance = totalPurchases - totalPayments;
    return { id: supplier.id, name: supplier.name, type: supplier.type, totalPurchases, totalPayments, balance };
  });
...
async function addAdvance(advance) {
  await db.read();
  const newAdvance = { id: Date.now(), employee_id: advance.employeeId, amount: advance.amount, advance_date: (/* @__PURE__ */ new Date()).toISOString() };
  (db.data.advances || []).push(newAdvance);
  await db.write();
  return newAdvance;
}
async function getEmployeeDebts() {
  await db.read();
  const employees = db.data.employees || [];
  const advances = db.data.advances || [];
  const employeeDebts = employees.map((employee) => {
    const totalAdvances = advances.filter((a) => a.employee_id === employee.id).reduce((sum, current) => sum + current.amount, 0);
    return { id: employee.id, name: employee.name, position: employee.position, totalAdvances };
  });
  return employeeDebts;
}
async function getClientStatement(clientId) {
  await db.read();
  const sales = db.data.sales || [];
  const payments = db.data.payments || [];
  const clientSales = sales.filter((sale) => sale.client_id === clientId).map((sale) => ({ date: sale.sale_date, type: "Savdo", description: `Sotilgan mahsulot`, amount: sale.total_price }));
  const clientPayments = payments.filter((payment) => payment.client_id === clientId).map((payment) => ({ date: payment.payment_date, type: "To'lov", description: "Mijoz tomonidan to'lov", amount: -payment.amount }));
  const statement = [...clientSales, ...clientPayments].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  return statement;
}
async function getFinancialSummary() {
  ...
  const supplierPayments = db.data.supplier_payments || [];
  const advances = db.data.advances || [];
  const totalCashIn = payments.reduce((sum, p) => sum + p.amount, 0);
  const totalCashOut = expenses.reduce((sum, e) => sum + e.amount, 0) + supplierPayments.reduce((sum, p) => sum + p.amount, 0) + advances.reduce((sum, a) => sum + a.amount, 0);
  const kassa = totalCashIn - totalCashOut;
  return { totalCashIn, totalCashOut, kassa };
}
async function getTransactions(filters) {
  await db.read();
  const { payments, expenses, supplier_payments, advances, clients, suppliers, employees } = db.data;
  const allTransactions = [];
  (payments || []).forEach((p) => {
    const client = (clients || []).find((c) => c.id === p.client_id);
    allTransactions.push({ date: p.payment_date, type: "Kirim", description: `To'lov: ${client ? client.name : "Noma'lum mijoz"}`, amount: p.amount });
  });
  (expenses || []).forEach((e) => {
    allTransactions.push({ date: e.expense_date, type: "Chiqim", description: `Xarajat: ${e.description}`, amount: -e.amount });
  });
  (supplier_payments || []).forEach((p) => {
    const supplier = (suppliers || []).find((s) => s.id === p.supplier_id);
    allTransactions.push({ date: p.payment_date, type: "Chiqim", description: `To'lov: ${supplier ? supplier.name : "Noma'lum chorvachi"}`, amount: -p.amount });
  });
  (advances || []).forEach((a) => {
    const employee = (employees || []).find((e) => e.id === a.employee_id);
    allTransactions.push({ date: a.advance_date, type: "Chiqim", description: `Avans: ${employee ? employee.name : "Noma'lum xodim"}`, amount: -a.amount });
  });
...
async function getSalesReport(filters) {
  ...
  filteredSales.forEach((sale) => {
    if (!report[sale.product_id]) {
      const product = products.find((p) => p.id === sale.product_id);
      report[sale.product_id] = { name: product ? product.name : "Noma'lum mahsulot", totalQuantity: 0, totalAmount: 0 };
    }
    report[sale.product_id].totalQuantity += sale.quantity;
    report[sale.product_id].totalAmount += sale.total_price;
  });
  return Object.values(report);
}
async function getTodaysFinancialSummary() {
  await db.read();
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const payments = (db.data.payments || []).filter((p) => p.payment_date.startsWith(today));
  const expenses = (db.data.expenses || []).filter((e) => e.expense_date.startsWith(today));
  const supplierPayments = (db.data.supplier_payments || []).filter((p) => p.payment_date.startsWith(today));
  const advances = (db.data.advances || []).filter((a) => a.advance_date.startsWith(today));
  const todayCashIn = payments.reduce((sum, p) => sum + p.amount, 0);
  const todayCashOut = expenses.reduce((sum, e) => sum + e.amount, 0) + supplierPayments.reduce((sum, p) => sum + p.amount, 0) + advances.reduce((sum, a) => sum + a.amount, 0);
  return { todayCashIn, todayCashOut, todayKassa: todayCashIn - todayCashOut };
}
async function exportDataToExcel() {
  await db.read();
  const data = db.data;
  const workbook = utils.book_new();
  const clientsSheet = utils.json_to_sheet(data.clients || []);
  utils.book_append_sheet(workbook, clientsSheet, "Mijozlar");
  const productsSheet = utils.json_to_sheet(data.products || []);
  utils.book_append_sheet(workbook, productsSheet, "Mahsulotlar");
  const salesSheet = utils.json_to_sheet(await getSales());
  utils.book_append_sheet(workbook, salesSheet, "Savdolar");
  const paymentsSheet = utils.json_to_sheet(await getPayments());
  utils.book_append_sheet(workbook, paymentsSheet, "Mijoz To'lovlari");
  const expensesSheet = utils.json_to_sheet(data.expenses || []);
  utils.book_append_sheet(workbook, expensesSheet, "Xarajatlar");
  const suppliersSheet = utils.json_to_sheet(data.suppliers || []);
  utils.book_append_sheet(workbook, suppliersSheet, "Chorvachilar");
  const purchasesSheet = utils.json_to_sheet(data.purchases || []);
  utils.book_append_sheet(workbook, purchasesSheet, "Mol Xaridi");
  const supplierPaymentsSheet = utils.json_to_sheet(data.supplier_payments || []);
  utils.book_append_sheet(workbook, supplierPaymentsSheet, "Chorvachilarga To'lov");
  const employeesSheet = utils.json_to_sheet(data.employees || []);
  utils.book_append_sheet(workbook, employeesSheet, "Xodimlar");
  const advancesSheet = utils.json_to_sheet(data.advances || []);
  utils.book_append_sheet(workbook, advancesSheet, "Avanslar");
...
electron.ipcMain.handle("get-clients", async () => await getClients());
electron.ipcMain.handle("add-client", async (event, client) => await addClient(client));
...
electron.ipcMain.handle("add-sale", async (event, sale) => await addSale(sale));
electron.ipcMain.handle("get-sales", async () => await getSales());
electron.ipcMain.handle("delete-sale", async (event, id) => await deleteSale(id));
electron.ipcMain.handle("add-payment", async (event, payment) => await addPayment(payment));
electron.ipcMain.handle("get-payments", async () => await getPayments());
electron.ipcMain.handle("delete-payment", async (event, id) => await deletePayment(id));
electron.ipcMain.handle("add-expense", async (event, expense) => await addExpense(expense));
electron.ipcMain.handle("get-expenses", async () => await getExpenses());
electron.ipcMain.handle("delete-expense", async (event, id) => await deleteExpense(id));
electron.ipcMain.handle("get-suppliers", async () => await getSuppliers());
electron.ipcMain.handle("add-supplier", async (event, supplier) => await addSupplier(supplier));
electron.ipcMain.handle("add-purchase", async (event, purchase) => await addPurchase(purchase));
electron.ipcMain.handle("add-supplier-payment", async (event, payment) => await addSupplierPayment(payment));
electron.ipcMain.handle("get-supplier-debts", async () => await getSupplierDebts());
electron.ipcMain.handle("get-client-statement", async (event, clientId) => await getClientStatement(clientId));
electron.ipcMain.handle("get-employees", async () => await getEmployees());
electron.ipcMain.handle("add-employee", async (event, employee) => await addEmployee(employee));
electron.ipcMain.handle("add-advance", async (event, advance) => await addAdvance(advance));
electron.ipcMain.handle("get-employee-debts", async () => await getEmployeeDebts());
electron.ipcMain.handle("get-financial-summary", async () => await getFinancialSummary());
electron.ipcMain.handle("get-transactions", async (event, filters) => await getTransactions(filters));
electron.ipcMain.handle("get-sales-report", async (event, filters) => await getSalesReport(filters));
electron.ipcMain.handle("get-todays-financial-summary", async () => await getTodaysFinancialSummary());
electron.ipcMain.handle("export-data", async () => {
  try {
    const buffer = await exportDataToExcel();
    const { filePath } = await electron.dialog.showSaveDialog({
      title: "Excel faylni saqlash",
      defaultPath: `hisobot-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.xlsx`,
